...
resource "azurerm_subnet" "subnet_apim_spk2" {
  name                 = "ApimSubnet"
  resource_group_name  = local.resource_group_name
  virtual_network_name = azurerm_virtual_network.spk2.name
  address_prefixes     = [format("%s%s", var.vnet_spk2_address_prefix, var.apim_subnet_address_suffix)]
}

resource "azurerm_subnet" "subnet_vm_spk2" {
  name                 = "VMSubnet"
  resource_group_name  = local.resource_group_name
  virtual_network_name = azurerm_virtual_network.spk2.name
  address_prefixes     = [format("%s%s", var.vnet_spk2_address_prefix, var.vm_subnet_address_suffix)]
}