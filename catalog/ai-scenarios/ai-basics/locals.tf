locals {
  ...
  resource_suffix_swc           = [lower(var.environment), "swc", lower(var.domain), lower(var.workload), random_id.resource_group_name_suffix.hex]
    ...
    tomap(
      {
        ...
        "ProjectName" = "ai-scenarios/ai-basics",
        "Domain"      = var.domain
      }
    )
  )
