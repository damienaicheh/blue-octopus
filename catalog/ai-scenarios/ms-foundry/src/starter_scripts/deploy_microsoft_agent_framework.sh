...
AGENT_NAME=writer-workflow-maf
...
az acr build --registry "$ACR_NAME" --image "$IMAGE_TAG" ../microsoft_agent_framework
...
# Agent already exists? If so, create a new version; otherwise, create the agent.
