...
resource "azurerm_private_endpoint" "ms_foundry_all_models" {
  name                = format("pe-aif-com-%s", local.resource_suffix_kebabcase)
  location            = local.resource_group_location
  resource_group_name = local.resource_group_name
  subnet_id           = azurerm_subnet.subnet_paas.id

  private_dns_zone_group {
    name = format("psc-%s", azapi_resource.ms_foundry_all_models.name)
    private_dns_zone_ids = [
      azurerm_private_dns_zone.privatelink_cognitiveservices_azure_com.id,
      azurerm_private_dns_zone.privatelink_services_ai_azure_com.id,
      azurerm_private_dns_zone.privatelink_openai_azure_com.id
  ...
  private_service_connection {
    name                           = format("psc-%s", azapi_resource.ms_foundry_all_models.name)
    private_connection_resource_id = azapi_resource.ms_foundry_all_models.id
    subresource_names = [
      "account"
    ]
    is_manual_connection = false
